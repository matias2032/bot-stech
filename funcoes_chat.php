...
// ─────────────────────────────────────────────────────────────
function chamarSmolLM(string $mensagem, array $contexto, string $id_conversa, PDO $pdo): array {
    ...
    // SmolLM2 responde melhor a instruções curtas e diretas — prompt simplificado
    $prompt_sistema  = "Ês {$nome_bot_str}, criado por {$nome_criador_str}. ";
    $prompt_sistema .= "Nunca digas que és da Google, Anthropic ou outro fornecedor. ";
    $prompt_sistema .= "Responde sempre em português, de forma curta e direta.\n";
    if (!empty($perfil['prompt_sistema'])) {
        $prompt_sistema .= $perfil['prompt_sistema'] . "\n";
    }

    $partes = $contexto['partes'] ?? [];
    if (!empty($partes)) {
        $prompt_sistema .= "\nContexto (usa apenas se relevante):\n"
            . implode("\n---\n", $partes);
    }
// else {
//         $prompt_sistema .= "\n\n## Base de Conhecimento\n"
...
$messages = [['role' => 'system', 'content' => $prompt_sistema]];
    foreach ($historico as $msg) {
        $messages[] = [
            'role'    => $msg['papel'] === 'utilizador' ? 'user' : 'assistant',
    ...
    if ($raw === false || $http_code !== 200) {
        error_log("[SmolLM2] Erro {$http_code}: " . substr((string)$raw, 0, 300));
        return [
    ...
    return [
        ...
        'erro'      => null,
        't_entrada' => $dados['prompt_eval_count'] ?? null,
        't_saida'   => $dados['eval_count']        ?? null,
        'tempo_ms'  => $tempo_ms,
    ];
}
